...
def main() -> int:
    ...
    for s in stories:
        ...
        if s.image_path and Path(s.image_path).exists():
            ...
            for m in (
                payload_copy.get("specificContent", {})
        ...
        if s.image_path and Path(s.image_path).exists():
            print(
                f"  - Register upload (assets API) for owner {Config.LINKEDIN_AUTHOR_URN}"
            )
            print(f"  - Upload binary image from: {s.image_path}")
        print("  - POST to /ugcPosts with payload above")
        print(
            "  - On success, update story.publish_status -> 'published' and store linkedin_post_id"
        )
