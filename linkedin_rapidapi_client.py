...
class HybridLinkedInLookup:
    """
    Hybrid LinkedIn lookup that tries multiple sources in order:
    1. Cache (fastest)
    2. Fresh LinkedIn Data API (most reliable)
    3. Browser-based fallback (slowest but always works)

    This achieves 90%+ success rate while minimizing API costs.
    """
    ...
    def __init__(
        ...
        browser_lookup: Optional[Any] = None,
    ):
        """
        Initialize hybrid lookup.

        Args:
            rapidapi_key: RapidAPI key for Fresh LinkedIn Data API
            cache: Cache object with get/set methods for storing results
            browser_lookup: Optional LinkedInCompanyLookup instance for fallback
        """
        self.cache = cache
        self.browser_lookup = browser_lookup

        # Initialize Fresh LinkedIn API client
        self.api_client = FreshLinkedInAPIClient(api_key=rapidapi_key, cache=cache)
    ...
    def lookup_person(
        ...
        Args:
            ...
            role: Optional role/title for better matching
            use_browser_fallback: Whether to try browser lookup if API fails

        Returns:
            Dict with 'linkedin_url', 'name', 'title', 'company', etc. or None
        """
        self._stats["total_lookups"] += 1

        # Try Fresh LinkedIn API first (includes cache check)
        result = self.api_client.search_person(name, company)
        ...
        # Try browser fallback if enabled and available
        if use_browser_fallback and self.browser_lookup:
            try:
                logger.debug(f"Trying browser fallback for {name} at {company}")
                browser_result = self.browser_lookup.find_person_linkedin(name, company)

                if browser_result and browser_result.get("linkedin_url"):
                    self._stats["browser_success"] += 1
                    browser_result["source"] = "browser"

                    # Cache the browser result for future use
                    if self.cache:
                        cache_key = self.api_client._get_cache_key(name, company)
                        self.cache.set(cache_key, browser_result, ttl=86400 * 30)

                    return browser_result

            except Exception as e:
                logger.debug(f"Browser fallback failed for {name}: {e}")

            self._stats["browser_failures"] += 1
    ...
    def lookup_batch(
        ...
        Args:
            people: List of dicts with 'name' and 'company' keys
            use_browser_fallback: Whether to try browser for API failures
            progress_callback: Optional callback(current, total, person) for progress

        Returns:
            Dict mapping "name|company" to result dict or None
        """
        results = {}
        total = len(people)
        ...
        for i, person in enumerate(people):
            ...
            if progress_callback:
                progress_callback(i + 1, total, person)

            key = f"{name}|{company}"
            results[key] = self.lookup_person(name, company, role, use_browser_fallback)

        return results

    def get_stats(self) -> Dict[str, Any]:
        """Get lookup statistics."""
        total = self._stats["total_lookups"] or 1
        success = self._stats["api_success"] + self._stats["browser_success"]
    ...
    def test_connection(self) -> Dict[str, bool]:
        ...
        return {
            "rapidapi": self.api_client.test_connection(),
            "browser": self.browser_lookup is not None,
        }
